...
export default function InvestigationPage() {
  ...
  return (
    <>
      <AdminPageHeader
        ...
        subtitle={
          caseData.title
            ...
            : "Kill chain timeline, evidence chain, and IOC extraction"
        }
        toolbar={
          cases.length > 1 ? (
            <select
      ...
      <AnimatedCard>
        <h2 className="mb-6 font-mono text-xs uppercase tracking-widest text-[var(--text-muted)]">
          Kill Chain Timeline
        </h2>
            ...
            return (
              ...
              >
                <div
                  ...
                  style={{
                    background: done ? "var(--violet)" : "var(--bg-surface)",
                    border: `2px solid ${done ? "var(--violet-light)" : "var(--border-default)"}`,
                  }}
                >
                  {done ? "✓" : i + 1}
                </div>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <AnimatedCard delay={0.2} className="lg:col-span-1">
          <h3 className="ac-section-title">Evidence Chain</h3>
          ...
          <TypewriterText
            text={
              ...
              (caseData.timeline ?? [])[0]?.text ??
              "Awaiting case timeline..."
            }
          />
          <div className="mt-4 flex gap-2">
            <input
              ...
              className="ac-form-control flex-1"
              style={{ borderRadius: 8 }}
            />
            <button type="button" onClick={onAddNote} className="btn-accent">
              Add
            </button>
          </div>
        </AnimatedCard>
        ...
        <AnimatedCard delay={0.3} className="lg:col-span-2">
          <h3 className="ac-section-title">Extracted IOCs</h3>
          <div className="space-y-2">
            {(iocs.length ? iocs : [{ type: "INFO", value: "No IOCs extracted yet", malicious: false }]).map(
              (ioc, i) => (
                <motion.div
