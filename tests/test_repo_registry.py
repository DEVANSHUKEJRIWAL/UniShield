...
from backend.connectors.repo_registry import (
    RepoRegistry,
    _enum_value,
    _from_pg_timestamp,
    _parse_enum,
...
from backend.schemas.repo_schemas import (
    RepoAuthMethod,
...
async def test_scan_all_repos_resolves_multiple(registry: RepoRegistry):
    ...
    with patch.object(
        ...
        from backend.schemas.repo_schemas import MultiRepoScanRequest
...
async def test_resolve_scan_target_falls_back_when_branch_wrong(registry: RepoRegistry):
    ...
    from backend.connectors.base_connector import BranchNotFoundError
